...
|---|---|---|
| ~~중복 제안~~ | **해결됨(5단계).** 살아 있는 환불은 주문당 하나다 — `REFUND-003@v1` 의 부분 유일 제약이 막는다. 서비스 층의 사전 조회는 **지웠다**. `propose_refund` 에 `request_id` 를 주면 재전송은 `outcome: REPLAYED`(그때의 답), 안 주면 `ALREADY`(지금 상태) | — |
| ~~경합~~ | **해결됨(5단계).** 두 창에서 동시에 시켜도 하나는 확정, 하나는 "이미 있다"를 듣는다. `just race refund <주문번호> --http --rounds 10` 으로 직접 쏴 볼 수 있다 | — |
| 잠금 실패 | 세계가 잠겨 판정하지 못하면 **503 + `Retry-After`** 다(5단계). 409 가 아니다 — 규칙 위반이 아니라 다시 보내면 될 요청이다. 빈도는 줄였지만 없어지지 않는다 | 8단계 — 운영 |
