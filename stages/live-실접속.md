...
|---|---|---|
| 중복 제안 | Claude 가 같은 주문에 두 번 제안하면 새 건이 아니라 기존 건의 상태가 돌아온다. 그건 **서비스 층이 걸러 주는 것**이지 세계(DB)가 막는 것이 아니다 | 5단계 — 멱등키 |
| 경합 | 그래서 두 창에서 **동시에** 시키면 조회와 생성 사이가 벌어져 둘 다 통과할 수 있다 | 5단계 |
