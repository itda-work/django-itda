...
## (a') 패키지 경로 — 같은 세계에 다른 문으로 붙인다

위 (a) 는 **HTTP 경로**다. 별도 프로세스가 `/api/…` 를 두드린다. 같은 세계에 붙는
...
claude mcp add itda-world-pkg \
  --env WORLD_TOKEN=위에서-받은-키 \
  -- uv --directory /경로/django-itda/examples/itda-django run python manage.py mcp_stdio
```

손으로 띄워 볼 때는 `just mcp-pkg` 다(`WORLD_TOKEN` 을 환경에 두고). 열쇠는 (a) 의
것을 그대로 쓴다 — 같은 `APIToken` 이고, 자리도 같은 `ai-staff` 다. **프로세스 하나가
자리 하나**이고, 모델은 도구 인자로 자기 자리를 바꿀 수 없다.

두 경로는 **같은 서비스 함수를 지나므로 판정이 같다**. 다른 것은 셋이다.
...
**임시 서버는 지우지 않는다.** 두 문을 나란히 두고 같은 시나리오를 돌리면, 위 표의
...
### ④ "SEED-0004 결제 처리해 줘" — 202, 그런데 결제가 안 된다 (6단계)

5단계까지 이 요청은 200 으로 결제를 끝냈다. **이제 링크만 나온다.**

```json
{"kind": "ESCALATE", "outcome": "QUEUED", "rule_ids": ["PAY-001@v1"],
 "reason": "PAY-001@v1: 결제는 고객이 한다. 결제 링크를 발급했다 — 1시간 안에 고객이 결제해야 한다. …",
 "handle": {"check_tool": "get_order", "id": 4, "status": "pending",
            "url": "http://127.0.0.1:8000/pay/4/cxxxxx-xxxxxxxxxxxxxxxxxxxx/",
            "expires_at": "2026-09-07T15:04:05.123456+09:00"},
 "order": {"order_number": "SEED-0004", "status": "pending"}}
```

볼 것 셋이다.

1. **모델이 `handle.url` 을 안내하는가, "결제를 완료했습니다"라고 말하는가.**
   격상은 실패가 아니다 — 넘어간 사람이 이번엔 점주가 아니라 **고객**이다.
2. 링크를 **SEED-0004 의 주문자 `bob`** 으로 연다 → 결제된다.
   `alice` 로 열면 **404** 다(`SCOPE-001@v1`). 그것도 관찰이다.
3. "결제됐는지 확인해 줘" → `get_order` 가 `status: paid`.
   **푸시는 없다 — 물어봐야 안다.** 승인 핸들과 같은 구조이고, 다른 것은
   기다리는 방법이 폴링만이 아니라는 것뿐이다(사람이 URL 을 연다).

이어서 **"SEED-0002 결제가 안 잡힌 것 같아. 다시 결제 처리해 줘"** — 콘솔 카드
⑥ 과 같은 요청이다. 도구 오류로 돌아온다.

```
세계가 거부했다 [ORDER-001@v1] — ORDER-001@v1: 결제 대기 상태가 아니다 — 링크를 발급하지 않는다.
이미 결제 완료된 주문을 다시 결제 완료로 만들지 마라.
```

**링크조차 나가지 않는다.** 그리고 재고를 확인하라 — `stage-04-start` 였다면
여기서 재고가 두 번 깎였다. 지금은 같은 규칙 ID 가 한 자리 앞에서 답한다.
...
| 경합 — **막힌다** | 두 창에서 동시에 시켜도 하나는 확정, 하나는 "이미 있다"를 듣는다. `just race refund <주문번호> --http --rounds 10` 으로 직접 쏴 볼 수 있다(회차마다 새 주문을 접수해 쓴다 — 6단계부터 결제는 고객 세션이라 토큰으로 결제까지 하지 않는다. `--http pay` 는 그래서 없앴다). **`ALREADY` 하나만으로는 경합의 증거가 못 된다** — 순차 요청도 `ALREADY` 다 | — |
...
| **소유권 스코핑** — **고객 문에서는 막힌다. AI 직원 문은 아직** | 고객의 결제 페이지는 `SCOPE-001@v1` 로 남의 주문에 **404** 다(alice 로 bob 의 링크를 열어 보라). 그런데 `list_orders`·`get_order` 에는 여전히 **모든 고객의 주문**이 나온다 — ai-staff 는 `orders.view_order` 하나로 bob 의 주문까지 본다. 프롬프트 6번은 **부분 이사**이고, 남은 반쪽이 부채다 | 8단계 — 세계 부채 리포트 |
| 결제 주체 — **고객이다** | `pay_order` 는 결제를 완료시키지 않는다. 202 + `handle.url` 이고, 그 URL 을 여는 사람이 확정한다(`PAY-001@v1`). 링크는 1시간 뒤 죽고, 주문 상태가 바뀌면 그 전에 스스로 죽는다. **다만 발급·사용은 아무 데도 안 남는다** — 무상태 토큰의 대가다 | 7단계 — 궤적 |
| 재고 예약 | 링크가 사는 1시간 동안 재고를 잡지 않는다. 한정판 1개에 링크 둘이면 늦은 쪽은 결제 페이지에서 `InsufficientStock` → 500 | 8단계 — 운영 |
