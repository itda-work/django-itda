...
> 교육 프로젝트는 **itda-django**(이 디렉터리, `examples/itda-django/`), 실행세계 도구면 패키지는 **django-itda**(같은 저장소의 루트, 임포트 이름 `django_itda`). 교육 프로젝트는 그 패키지의 첫 사용자이자 예시로 uv workspace 멤버다. 이름이 거울상이라 적어 둔다. 2026-09-06 까지의 이름은 hyve-django 였다.
