...
- 위치 `~/Apps/itda-work/django-itda/examples/itda-django` (2026-09-06 구조 뒤집기로 하위 이동, 그 전에는 저장소 루트. 같은 날 `hyve-django` 에서 `itda-django` 로 개명). 도구면 패키지 **django-itda** 는 저장소 루트(`django_itda`, 임포트 이름도 `django_itda`, 자체 `CLAUDE.md`)이고 이 프로젝트는 그 uv workspace 멤버이자 첫 사용자 — 교육 프로젝트가 itda-django, 패키지가 django-itda. 프로젝트 지식은 저장소 안에 자족적으로 둔다(개인 메모리 의존 0).
- 툴 파라미터의 한글은 리터럴 UTF-8. `\uXXXX` 이스케이프 금지.
- 스택: Python 3.12+ · uv · Django 5.2 LTS ~ 6.1 · SQLite · pytest-django · ruff. 8단계 전 외부 서비스 0.
...
코드 구현은 저장소 루트의 `.claude/agents/app-builder.md`(Opus · medium)에 위임한다. 메인 세션 상위 모델로 직접 구현하지 않는다.
