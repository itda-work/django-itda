...
| `LEDGER-001@v1` | 세계를 움직인 일은 빠짐없이 장부에 남겨라 — 누가·언제·어느 문으로·어떤 규칙으로. 장부는 고치지 마라. | `agent/prompts/ai_staff.md` 7번 (점주, 2026-09-07) | 전이 메서드 안의 기록(같은 트랜잭션) + **DB 트리거**(append-only) + 모델 층 거부 | `ledger/models.py:Event.record`·`Event.save`/`delete`·`EventQuerySet.delete` · `ledger/migrations/0002_append_only_trigger.py` · `orders/models.py`(`Order.mark_paid`·`Refund.apply`/`approve`/`reject`) · `orders/services.py`(`intake_order`·`issue_payment_link`) · `ledger/admin.py` | `tests/stage_07_ledger.py::test_거부된_결제는_장부에_없다` · `::test_같은_트랜잭션이다` · `::test_장부는_고칠_수_없다_DB_층` · `::test_admin_승인은_장부에_남는다_LogEntry는_여전히_0` | active | 시행 면이 셋이고 보장 범위가 면마다 다르다. ① **남기기** — 전이 메서드 안, 사실 뒤, 같은 트랜잭션이다. **적용**: `mark_paid`·`apply`/`approve`/`reject` 경로(admin·shell·도구면 어디서 불러도 지난다). **우회 가능**: `Order.objects.filter(...).update(status='paid')` 처럼 전이 메서드를 지나지 않는 직접 UPDATE 는 장부에 안 남는다. ② **갱신 금지** — `BEFORE UPDATE` 트리거라 **경로 무관**이다. Django 밖의 raw SQL 도 `IntegrityError`(`SQLITE_CONSTRAINT`)로 걸린다. ③ **삭제 금지** — 모델 층(`Event.delete`·`EventQuerySet.delete`)뿐이다. **우회 가능**: `DELETE FROM ledger_event` 는 막지 않는다 — Django 의 테스트 DB flush 가 그 문장을 쓰기 때문에 트리거를 걸면 teardown 이 깨진다. **SQL 로 열려 있는 것은 삭제만이 아니다**(실측 2026-09-07): `INSERT OR REPLACE` 는 같은 PK 의 내용을 통째로 교체하고(UPDATE 가 아니라 트리거를 지나지 않는다), 직접 `INSERT` 는 일어나지 않은 사실을 장부에 더하며, `DROP TRIGGER` 권한이 있으면 갱신 금지 자체를 걷어낼 수 있다. 여기에 ① 의 우회로(전이 밖 직접 UPDATE 는 아무 기록도 남기지 않는다)를 더한 넷이 현재 열린 범위다. 넷 다 DB 역할 권한의 문제이고(PostgreSQL `REVOKE DELETE`·`REVOKE TRIGGER`) 8단계다. **계정 삭제** — 장부에 actor 로 남은 계정은 지울 수 없다(`on_delete=PROTECT` → `ProtectedError`). 익명화·삭제 정책은 8단계다. **해석 주의** — 도메인 장부에는 **세계가 바뀐 사실만** 적는다. 거부(DENY)·재전송(REPLAYED)·기존 건 응답(ALREADY)은 행이 없다(불린 사실은 도구면 궤적 `django_itda.ToolCall` 의 몫이고, API·콘솔 문의 호출 궤적은 아직 없다 — 8단계). `Event.reason` 에는 **세계의 판정 문장만** 넣는다(고객·모델의 원문은 `Refund.reason` 에). 상관 ID(`X-Request-ID`)는 형식만 검사하고 **믿는다** |
...
- **7단계** — 일곱 번째 행이 생겼고, 이 행은 **법이 무엇을 지키는가**가 아니라
  **법이 지켜졌다는 사실을 어떻게 남기는가**를 말한다. 시행 면이 셋인 것은
  `REFUND-003@v1` 과 같지만 성질이 다르다 — 저기는 경합의 모양이 셋이었고,
  여기는 **장부가 거짓이 되는 방식이 셋**이다(안 남는다·고쳐진다·지워진다).
  세 면 중 갱신 금지만 경로 무관이고, 나머지가 SQL 로 어디까지 열려 있는지
  (삭제·`INSERT OR REPLACE`·직접 `INSERT`·트리거 제거)를 위 칸에 그대로 적었다.
  `PROTECT` 로 정한 계약도 거기 있다 — **장부에 남은 계정은 지울 수 없다.**
  장부 불변과 계정 삭제는 동시에 성립하지 않아서(삭제가 장부 행을 UPDATE 한다)
  둘 중 하나를 골라야 했고, 여기서는 장부가 이긴다.
  같이 켜진 `Order.paid_at` 의 `CheckConstraint`(`order_paid_has_paid_at`)는
  **대장 행이 아니다** — 점주가 말한 문장이 아니라 범용 제약이고, 0단계의
  `PositiveIntegerField` 와 같은 격이다. `REFUND-001@v1` 은 여전히
  `created_at` 을 결제일로 본다. `paid_at` 이 생겼어도 규칙이 그것을 읽게
  바꾸는 것은 `REFUND-001@v2` 라는 **새 행**이 필요한 일이라 뒤로 미룬다.
