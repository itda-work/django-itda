...
- **문의 모양** — 같은 세계를 MCP · 스킬+HTTP · CLI 중 어떤 면으로 내놓는 것이 좋은가. 2026-09-07 부터 실측 트랙: [docs/인터페이스-비교-MCP-스킬HTTP-CLI.md](docs/인터페이스-비교-MCP-스킬HTTP-CLI.md)(비교 축·실험 설계·도메인 후보·이 패키지에 주는 사양 입력). 결론이 모이면 위 5번(브리지)을 그 결론으로 고쳐 쓴다.
